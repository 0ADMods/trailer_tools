...
class CombatDemoHugeScript {
    constructor() {
        this.cinemaManagerScript = new CinemaManagerScript("Mainland R28");
        this.waypoints = [];
    }
    ...
    Start() {
        ...
        for (let i = 0; i < numOfBlocks; ++i) {
            ...
            const enemyBlock = this.CreateBlock("units/germ/infantry_spearman_e", 1, 3 * Math.PI / 2, mx + sep + 6, mz, 16, 4, 2.1, 3.0);
            cmpTrigger.DoAfterDelay(1000, "SendMoveCommand", {
                "queued": false,
                "waypoint": new Vector2D(mx + 10, mz),
                "entities": enemyBlock,
            ...
            let enemyRanged = this.CreateBlock("units/germ/infantry_javelineer_e", 1, 3 * Math.PI / 2, mx + sep, mz, 8, 1, 5.0, 3.0);
            enemyRanged = enemyRanged.concat(this.CreateBlock("units/germ/infantry_javelineer_e", 1, 3 * Math.PI / 2, mx + sep + 2, mz, 7, 1, 5.0, 3.0));
            this.SetStance({
                "stance": "standground",
                "owner": 1,
...
binaries/system/pyrogenesis \
    ...
    -mod=public \
    -mod=trailer_tools \
    -autostart=random/mainland_battle_r28 \
    -autostart-disable-replay \
    -autostart-player=-1 \
    -autostart-size=384 \
    -autostart-civ=1:germ \
    ...
    -autostart-players=8 \
    -autostart-biome='generic/alpine' \
    -autostart-seed=0 \
